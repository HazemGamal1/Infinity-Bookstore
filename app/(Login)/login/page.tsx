import React from 'react'
import Image from 'next/image'
import Link from 'next/link'
import logo from '@/app/logoBlack.png'

const Page = () => {
  return (
    <div className='grid w-screen h-screen place-content-center'>
      <Link href="/">
        <div className='font-bold text-2xl cursor-pointer absolute left-2 bottom-2'><Image src={logo} alt="logo" width={50}/></div>
      </Link>
      <div className='shadow-2xl bg-[#FFF] pt-8 pb-6 px-8 relative rounded-2xl lg:w-[25vw] mx-2'>
        <div className='mb-6'>
            <div className='mb-4'>
                <h1 className='mb-2 text-sm lg:text-lg'>Username</h1>
                <input type="text" name="" id="" className='rounded-lg px-4 py-1 lg:py-2 w-full border-2  focus:border-2'/>
            </div>
            <div>
                <h1 className='mb-2 text-sm lg:text-lg'>Password</h1>
                <input type="password" name="" id="" className='rounded-lg px-4 py-1 lg:py-2 w-full border-2  focus:border-2'/>
            </div>
        </div>
        <div className='w-full'>
            <div className='mx-auto w-full text-center font-bold bg-black text-white hover:bg-hover-color duration-300 py-2 lg:py-2 rounded-lg cursor-pointer'>
                <h1 className='mx-auto text-sm lg:text-sm'>Sign In</h1>
            </div>
        </div>
        <div className='mt-6 text-sm lg:text-lg'>
          <p>Don't have an Account? <span className='cursor-pointer text-hover-color'><Link href="/sign-up">Sign Up</Link></span></p>
        </div>
      </div>
    </div>
  )
}

export default Page
